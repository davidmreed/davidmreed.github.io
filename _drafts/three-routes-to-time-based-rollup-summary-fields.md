...
Most naive solutions to this class of requirements fail because they're based on formula fields or date literals. One might try creating a formula like this, for example:
...
The same underlying limitation applies for both of these restrictions: native Rollup Summary Fields require a *triggerable event* on the rolled-up object in order to update the rolled-up value. Rollup Summary Fields are evaluated as part of the [Trigger Order of Execution](https://developer.salesforce.com/docs/atlas.en-us.apexcode.meta/apexcode/apex_triggers_order_of_execution.htm#!) (steps 16 and 17).

Formula fields, though, don't have any underlying storage - their values are generated at the time the record if viewed. For this reason, there's no event when a formula field's value "changes" - in fact, some formula fields can be nondeterministic and incorporate date or time fields, such that their value is essentially always "changing".

Likewise, there's no triggerable event when the current day changes from one to the next. That's why we cannot embed date literals in our Rollup Summary Field criteria - when today ticks over to tomorrow, not only is there no triggerable event, but a recalculate job could be very computationally expensive.

The approaches we will take to solve this requirement ultimately take both tacks: creating a triggerable event based on the date, and arranging for broad-based recalculation on a schedule.

## Approach 1: Native Rollup Summary Fields with Time-Based Workflow or Process

Our first approach maximizes use of out-of-the-box functionality, requiring no Apex and no additional packages, but scales poorly and is best suited for smaller Salesforce orgs.

We can't create a native Rollup Summary Field, as we've seen, based on a formula field. But we can rollup based upon a Checkbox field, and we can use Time-Based Workflow and Processes to update that Checkbox on a schedule, using a couple of clever formula fields to get the dates right.
...
Here's how it works:
 ...
 1. We create a Rollup Summary Field on the parent object (here, Account). We use the criterion that our Checkbox field is set to True.

![Rollup Summary Field]({{ "/public/time-based-rollups/Rollup Summary Field.png" | absolute_url }})

 1. We create a Workflow Rule with two Time-Based Actions attached to it.
...
This solution, as noted above, is best suited for smaller Salesforce orgs with limited data sets and small numbers of time-based rollups. Each time-based rollup will consume three custom fields on the child and one Rollup Summary Field on the parent, as well as a Process or a Workflow Rule with two attached Actions.
...
More important, though, is the platform limitation on throughput of time-based workflow actions. From [](https://help.salesforce.com/articleView?id=workflow_time_action_considerations.htm&type=5):
...
That immediately poses us a problem. When the clock ticks over to June 1, for example, we suddenly have 30,000 emails that just went out of scope for our "Activities Logged This Month" rollup. Salesforce now begins processing the 30,000 enqueued workflow actions that have accumulated in the queue, but it can only process 1,000 per hour. Worse, the limit is 1,000 per hour *across the whole org* - not per workflow. So we now have a backlog 30 hours deep that impacts both our Rollup Summary Field *and* any other functionality based on timed actions across our org.
...
To start with, this Rollup Summary will only work partially. It'll yield correct results if we run a full calculate job by clicking `Calculate`, and if we configure it to run in `Realtime` mode, we'll see our rollup update as we add and delete Opportunities. 

What won't work, though, is the shift from one time period to the next. On the first of the month, all of our Opportunities from last month are still rolled up - there was no triggerable event that would cause them to be reevaluated against our criteria.

With DLRS, rather than using time-based workflow actions to create a triggerable event by updating a field, we recalculate the rollup value across every parent record when each time period rolls over. Here, we've scheduled a full recalculation of the rollup for the first day of each month.
...
Continuing the example developed above, where we're rolling up Opportunities for the current month only, our batch class's `start()` method would run a query like this:
    ...
    FROM Opportunity
    WHERE (CloseDate = THIS_MONTH AND This_Month_Batch__c = false) OR (CloseDate != THIS_MONTH AND This_Month_Batch__c = true)
...
Organizations with high data volume or data skew may need to experiment carefully to ensure the class can successfully execute.

As with DLRS, each rollup using this strategy consumes one scheduled Apex job.